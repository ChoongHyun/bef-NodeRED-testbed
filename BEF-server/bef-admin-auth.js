...
module.exports = {
    ...
    // cookie validation
    getValidation: function( cookie, nodeRedUrl, appParam, path, response, next ) {
        // applicationId 유무
        if (appParam != null) {
            ...
            if (cookie['bef-login-token'] != null || cookie.value != null) {
                ...
                // applicationId는 있으나 bef-login-cookie 값이 없을경우 noauth
                return noauthPage(response);
            }
        // applicationId 없으면 portal에서 조회
        }else if( path == '/' && appParam == null ){
            this.getCookie( path, nodeRedUrl, response, next );
        }else{
            // login page 일 경우
            if( path == '/auth/login' ){
                return noauthPage( response );
...
function decodeToken( token, appParam ){
	var decode = new Buffer(token.value, 'base64');
