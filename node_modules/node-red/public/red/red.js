        ...
        if (mouse_mode != RED.state.JOINING) {
            ...
            nodeEnter.each(function(d,i) {
                    ...
                    var mainRect = node.append("rect")
                        ...
                        .attr("ry", 5)
                        // .attr("fill",function(d) { return d._def.color;})
                        .attr("stroke",function(d) { return d._def.color;})
                        .on("mouseup",nodeMouseUp)
                        .on("mousedown",nodeMouseDown)
                        .on("touchstart",function(d) {
                            var obj = d3.select(this);
                    ...
                    if (d.dirty) {
                        ...
                        if (mouse_mode != RED.state.MOVING_ACTIVE) {
                            ...
                            if (d.inputs === 0 && !inputPorts.empty()) {
                                ...
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10).attr("stroke",function(d){ return d._def.color;})
                            ...
                            var rgbOutput = d._def.color;
                            var outputStroke = "";
                            if ( rgbOutput == null || rgbOutput.trim() == '' || !(rgbOutput.includes('rgb')) || rgbOutput.search("rgb") == -1 ) {
                                outputStroke = rgbOutput;
                            } else {
                                rgbOutput = rgbOutput.match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/);
                                outputStroke = "#" + ("0" + parseInt(rgbOutput[1]).toString(16)).slice(-2) + ("0" + parseInt(rgbOutput[2]).toString(16)).slice(-2) + ("0" + parseInt(rgbOutput[3]).toString(16)).slice(-2);
                            }
                            ...
                            output_group.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10).attr("stroke", outputStroke)
