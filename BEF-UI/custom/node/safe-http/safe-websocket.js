...
module.exports = function(RED) {
    ...
    function WebSocketListenerNode(n) {
        ...
        if (node.isServer) {
            ...
            var verifyClient = function (info, cb) {
                var authHeader = info.req.headers.authorization;
                console.log("verify client - url : " + info.req.url);
                for (var attr in info.req.headers) {
                    console.log("header " + attr + " : " + info.req.headers[attr] );
                }
                ...
                if (authHeader) {
                    ...
                    cb(false, 401, 'Unauthorized : token is required.');
                    //cb(true);
                }
            };

            serverOptions.verifyClient = verifyClient;
    ...
    RED.nodes.registerType("safe-websocket-listener",WebSocketListenerNode);
    RED.nodes.registerType("safe-websocket-client",WebSocketListenerNode);
    ...
    RED.nodes.registerType("safe-websocket in",WebSocketInNode);
    ...
    RED.nodes.registerType("safe-websocket out",WebSocketOutNode);
}
